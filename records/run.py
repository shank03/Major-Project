...
class Runner():
    ...
    def configure_topo(self):
        ...
        s0 = topo.addSwitch('s0', log_file="%s/%s.log" %(self.log_dir, 's0'), cls=self.p4_switch_classes['l2'])
        s1 = topo.addSwitch('s1', log_file="%s/%s.log" %(self.log_dir, 's1'), cls=self.p4_switch_classes['l2'])
        s2 = topo.addSwitch('s2', log_file="%s/%s.log" %(self.log_dir, 's2'), cls=self.p4_switch_classes['l2'])
        s3 = topo.addSwitch('s3', log_file="%s/%s.log" %(self.log_dir, 's3'), cls=self.p4_switch_classes['l2'])
        s4 = topo.addSwitch('s4', log_file="%s/%s.log" %(self.log_dir, 's4'), cls=self.p4_switch_classes['l2'])
    ...
    def fill_arp_table(self, host_name):
        hosts=[('h10', "192.168.1.2", "00:00:00:00:01:00"),
