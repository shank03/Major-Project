...
class CustomTopo(Topo):
    ...
    def __init__(self, *args, **kwargs):
        ...
        h1 = self.addHost(
            "h10", cls=IPv4Host, ip="192.168.1.2/24", mac="00:00:00:00:01:00"
        )
        h2 = self.addHost(
            "h11", cls=IPv4Host, ip="192.168.1.3/24", mac="00:00:00:00:02:00"
        )
        h3 = self.addHost(
            "h20", cls=IPv4Host, ip="192.168.1.4/24", mac="00:00:00:00:03:00"
        )
        h4 = self.addHost(
            "h21", cls=IPv4Host, ip="192.168.1.5/24", mac="00:00:00:00:04:00"
        )
        h5 = self.addHost(
            "h30", cls=IPv4Host, ip="192.168.1.6/24", mac="00:00:00:00:05:00"
        )
        h6 = self.addHost(
            "h31", cls=IPv4Host, ip="192.168.1.7/24", mac="00:00:00:00:06:00"
        )
        h7 = self.addHost(
            "h40", cls=IPv4Host, ip="192.168.1.8/24", mac="00:00:00:00:07:00"
        )
        h8 = self.addHost(
            "h41", cls=IPv4Host, ip="192.168.1.9/24", mac="00:00:00:00:08:00"
        )
        # self.hosts = [h1, h2, h3, h4, h5, h6, h7, h8]

        s0 = self.addSwitch("s0", cls=StratumBmv2Switch, cpuport=CPU_PORT, extModule='/extern/cpu.so')
        s1 = self.addSwitch("s1", cls=StratumBmv2Switch, cpuport=CPU_PORT, extModule='/extern/cpu.so')
        s2 = self.addSwitch("s2", cls=StratumBmv2Switch, cpuport=CPU_PORT, extModule='/extern/cpu.so')
        s3 = self.addSwitch("s3", cls=StratumBmv2Switch, cpuport=CPU_PORT, extModule='/extern/cpu.so')
        s4 = self.addSwitch("s4", cls=StratumBmv2Switch, cpuport=CPU_PORT, extModule='/extern/cpu.so')
